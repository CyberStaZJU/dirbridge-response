...
def encode_feature_from_delta(state, args, delta) -> torch.Tensor:
...
def encode_client_feature(state, args, idx: int) -> torch.Tensor:
    return encode_feature_from_delta(state, args, state['delta'][idx])
...
def _buffer_delay_limit(args) -> float:
    override = _getattr(args, 'dirbridge_buffer_delay_limit', None)
...
def should_rebuild_groups(state, args) -> bool:
    # E2 online start: the first grouping is created as soon as the first
    # wave of updates has arrived, rather than waiting for the regular
    # reclustering interval.
    if state.get('e2_online_bootstrap_pending', False):
        state['last_group_rebuild_reason'] = 'online_bootstrap'
        return True
...
def rebuild_direction_groups(state, args):
    ...
    # E2: in the full warm-start run every client direction is known; in the
    # online run only clients whose update has actually arrived may enter the
    # clustering. Features are never back-filled by training an unseen client.
    seen_set = state.get('seen_set')
    if seen_set is None:
        clustered_idx = list(range(num_users))
    else:
        clustered_idx = [idx for idx in range(num_users) if idx in seen_set]

    if len(clustered_idx) < 2:
        state['last_group_rebuild_reason'] = 'insufficient_seen_clients'
        state['e2_clustered_clients'] = len(clustered_idx)
        return

    state['e2_online_bootstrap_pending'] = False
    ...
    embeds = [None] * num_users
    group_ids_full = [-1] * num_users
    for pos, idx in enumerate(clustered_idx):
        embeds[idx] = features[pos].clone()
        group_ids_full[idx] = int(group_ids[pos])
    state['client_embeds'] = embeds
    state['group_ids'] = group_ids_full
    state['group_members'] = [[] for _ in range(actual_num_groups)]
    state['group_counts'] = [0] * actual_num_groups

    for pos, idx in enumerate(clustered_idx):
        gid = int(group_ids[pos])
        state['group_members'][gid].append(idx)
        state['group_counts'][gid] += 1

    state['last_group_rebuild'] = int(state['iterations'])

    # E5 diagnostic: assignment stability vs the previous grouping (greedy max
    # centroid-cosine match), non-empty group count, intra-group mean pairwise
    # cosine similarity (lower spread = tighter groups).
    prev_centroids = state.get('prev_rebuild_centroids')
    try:
        if prev_centroids is not None and group_centroids is not None:
            sims = group_centroids @ prev_centroids.t()
            stability = float(sims.max(dim=1).values.mean().item())
        else:
            stability = float('nan')
    except Exception:
        stability = float('nan')
    state['last_group_stability'] = stability
    state['last_nonempty_groups'] = int(sum(1 for c in state['group_counts'] if c > 0))
    try:
        spreads = []
        embeds = state['client_embeds']
        for members_list in state['group_members']:
            if len(members_list) >= 2:
                feats = torch.stack([embeds[i] for i in members_list])
                sims_m = feats @ feats.t()
                n = feats.shape[0]
                off = (sims_m.sum() - sims_m.diag().sum()) / (n * (n - 1))
                spreads.append(float(off.item()))
        state['last_mean_intra_group_sim'] = float(sum(spreads) / len(spreads)) if spreads else float('nan')
    except Exception:
        state['last_mean_intra_group_sim'] = float('nan')
    # E5 diagnostic: inter-group separation (mean pairwise cosine between distinct
    # group centroids). Reported alongside the intra-group value so the K0 sweep can
    # be read as within-group tightness vs between-group separation, not accuracy alone.
    try:
        c = group_centroids
        if c is not None and c.shape[0] >= 2:
            sims_c = c @ c.t()
            m = c.shape[0]
            state['last_mean_inter_group_sim'] = float(
                ((sims_c.sum() - sims_c.diag().sum()) / (m * (m - 1))).item()
            )
        else:
            state['last_mean_inter_group_sim'] = float('nan')
    except Exception:
        state['last_mean_inter_group_sim'] = float('nan')
    state['prev_rebuild_centroids'] = group_centroids.clone()
    state['prev_rebuild_group_ids'] = list(group_ids_full)
    state['e2_clustered_clients'] = len(clustered_idx)

    _update_e2_group_weights(
        state, args, clustered_idx, group_ids, group_centroids
    )
...
def _update_e2_group_weights(state, args, clustered_idx, group_ids, group_centroids):
    ...
    num_users = int(args.num_users)
    num_groups = int(state['num_groups'])
    source = e2_weight_source(args)
    online = e2_online_init(args)

    if online and source == 'full_count':
        raise ValueError(
            'E2: --e2_init_mode online cannot use --e2_weight_source '
            'full_count, which needs the directions of clients that have '
            'not arrived yet'
        )

    counts = [0.0] * num_groups
    if source == 'arrival_freq':
        arrival = state.get('arrival_count') or [0] * num_users
        for pos, idx in enumerate(clustered_idx):
            counts[int(group_ids[pos])] += float(max(0, int(arrival[idx])))
        weights = normalize_counts(counts)
    elif source == 'unique_client':
        for pos in range(len(clustered_idx)):
            counts[int(group_ids[pos])] += 1.0
        weights = normalize_counts(counts)
    elif source == 'oracle':
        weights = None
    else:
        for gid in state['group_ids']:
            if 0 <= int(gid) < num_groups:
                counts[int(gid)] += 1.0
        weights = normalize_counts(counts)

    oracle_weights = oracle_group_weights(
        state.get('oracle_features') or [None] * num_users,
        group_centroids,
        num_groups,
    )
    if weights is None:
...
def _update_e2_metrics(state, args):
    """Coverage and weight-error diagnostics for the online variants."""
    num_users = len(state['delta'])
    num_groups = int(state['num_groups'])
    seen_set = state.get('seen_set') or set()
    seen_flags = [idx in seen_set for idx in range(num_users)]
    num_seen = int(sum(1 for flag in seen_flags if flag))

    metrics = {
        'seen_clients': num_seen,
        'seen_ratio': float(num_seen) / float(num_users) if num_users else float('nan'),
    ...
    metrics['group_refresh_gap_mean'] = group_refresh_gap(
        state.get('group_members') or [],
        state.get('e2_client_last_arrival') or [],
        int(state['iterations']),
    )
    state['e2_metrics'] = metrics


def _populate_oracle_features(state, args):
    """One-time full-population direction snapshot at w_0.

    Measurement and oracle-weight instrument only: it never feeds the
    clustering and never writes into the per-client deltas.
    """
...
def _build_ema_group_updates(state, args, buffer_list):
    ...
    stored_weights = state.get('group_weights')
    for group_id in range(num_groups):
        group_size = max(0, int(group_counts[group_id])) if group_id < len(group_counts) else 0
        if stored_weights is not None and group_id < len(stored_weights):
            group_weight = float(stored_weights[group_id])
        else:
...
def _schedule_new_clients(state, args, active_list, num_to_launch, random_cost):
    ...
    for idx in sampled_idx:
        delta = local_train_delta(state, args, idx)
        feature = encode_feature_from_delta(state, args, delta)
        launch_stamp = int(state['iterations'])
        arrival_cost = state['global_cost'] + random_cost(idx)
        inflight[idx] = {
            'delta': delta,
            'feature': feature,
            'stamp': launch_stamp,
        }
        state['stamp'][idx] = launch_stamp
        state['cost'][idx] = arrival_cost


def _materialize_arrivals(state, buffer_list):
    """Make completed in-flight results visible to the server."""
    inflight = state.setdefault('inflight', {})
    for idx in buffer_list:
        record = inflight.pop(idx, None)
        if record is None:
            raise RuntimeError(
                f'Client {idx} arrived without an in-flight result'
            )
        state['delta'][idx] = record['delta']
        state['client_features'][idx] = record['feature']
        state['stamp'][idx] = int(record['stamp'])
...
def init_state(state, args, random_cost):
    ...
    state['group_weights'] = [0.0] * state['num_groups']
    ...
    # E2 bookkeeping is present in both start modes so the state schema and
    # the diagnostics are identical across the control matrix.
    online_init = e2_online_init(args)
    state['e2_init_mode'] = 'online' if online_init else 'full'
    state['e2_weight_source'] = e2_weight_source(args)
    # A full warm start computes every client direction anyway, so the
    # population snapshot is free and is always kept; the online start keeps
    # it only when requested, since it costs an extra full-client pass.
    state['e2_oracle_enabled'] = bool(e2_oracle_enabled(args)) or not online_init
    state['oracle_features'] = [None] * num_users
    state['arrival_count'] = [0] * num_users
    state['e2_client_last_arrival'] = [-1] * num_users
    state['seen_set'] = set()
    state['e2_metrics'] = {}
    ...
    if online_init:
        # Deployable start: only the first wave of clients is launched, and
        # no client direction is known before its update arrives.
        state['last_grouping_method'] = 'none'
        state['e2_online_bootstrap_pending'] = True
        _init_empty_ema_group_cache(state)
        if state['e2_oracle_enabled']:
    ...
    state['seen_set'] = set(range(num_users))
    if state['e2_oracle_enabled']:
        for idx in range(num_users):
            state['oracle_features'][idx] = state['client_features'][idx].clone()
    ...
    for idx in sampled_idx:
        state['inflight'][idx] = {
            'delta': state['delta'][idx],
            'feature': state['client_features'][idx],
            'stamp': int(state['stamp'][idx]),
        }
