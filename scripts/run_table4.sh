...
# Table 4: simulated-time convergence and server-side overhead on CIFAR-10.
TABLE="table4"
ALGOS="${ALGOS:-DirBridge FedBuff FedBuffMALight CASA CA2FL FADAS FedASMU}"
SYSTEM_ROOT="${SYSTEM_ROOT:-${OUT_ROOT}/system_metrics/table4}"
mkdir -p "${SYSTEM_ROOT}"
for seed in ${SEEDS}; do
  for algo in ${ALGOS}; do
    run_one "${TABLE}" "cifar_alpha0.5" "cifar" "${algo}" "${algo}" "${seed}"       --distribution noniid --alpha 0.5 --random_cost mild_label_correlated_hierarchical       --num_users 100 --concurrency 40 --buffer_size 10 --total_rounds 500       --model resnet --lr 0.01 --local_bs 100 --local_period 10 --interval 1       --system_metrics_log_dir "${SYSTEM_ROOT}/${algo}/seed=${seed}"
  done
done
